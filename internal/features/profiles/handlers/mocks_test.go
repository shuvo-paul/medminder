...
func (m *MockProfileService) CreateProfile(ctx context.Context, ownerUserID uuid.UUID, name string, dateOfBirth *time.Time, timezone string, schedules []service.DoseScheduleInput) (*service.ProfileResult, error) {
	args := m.Called(ctx, ownerUserID, name, dateOfBirth, timezone, schedules)
	if args.Get(0) == nil {
