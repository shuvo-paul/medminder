...
8. WhatsApp and Telegram notifications with retry logic
...
- `id`, `profile_id` (FK → Profile), `whatsapp_enabled` (boolean), `whatsapp_phone` (nullable), `telegram_enabled` (boolean), `telegram_chat_id` (nullable), `quiet_hours_start` (nullable, `HH:MM`), `quiet_hours_end` (nullable, `HH:MM`), `advance_minutes` (integer, default 0)
